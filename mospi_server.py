...
def get_metadata(
    ...
    try:
        ...
        if dataset == "CPI":
            ...
            indicator_code_int, err = _safe_int(indicator_code, "indicator_code")
            if err:
                return err
            if indicator_code_int < 1 or indicator_code_int > 29:
                return {"error": "indicator_code must be between 1 and 29 for NSS81."}
            result = mospi.get_nss81_filters(indicator_code=indicator_code_int)
            result["api_params"] = get_swagger_param_definitions("NSS81")
            result["parameter_notes"] = (
                "indicator_code selects the specific indicator (1-29). "
                "Filters depend on the indicator's domain (employment, education, health)."
            )
            result["next_step"] = _next
            return _check_empty_metadata(result, dataset, indicator_code=indicator_code_int)
        else:
            return {"error": f"Unknown dataset: {dataset}", "valid_datasets": VALID_DATASETS}
...
def list_datasets() -> dict:
    ...
    return {
        ...
        "datasets": {
            ...
            "NSS81": {
                "name": "NSS 81st Round - Modular Survey",
                "description": "Annual modular survey covering employment and education.",
                "use_for": "employment, education, health",
            },
        },
        "workflow": [
